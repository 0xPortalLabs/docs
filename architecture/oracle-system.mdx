---
title: "Oracle System"
description: "Cryptographic verification of off-chain points"
---

# Oracle System

The Oracle is Checkpoint's verification layer. It queries off-chain points APIs, generates
...
- Verify that a trade was properly settled on-chain and get a signature for a crosschain settlement
  transaction
...
When your points balance changes, the oracle records this on-chain via `changePoints()`.

<Info> This usually happens every 24 hours but there is no strong guarantee. </Info>

All data is queryable on-chain via the Soulbound NFT (DepositReceipt.sol) and is tracked internally
using
