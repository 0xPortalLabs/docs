...
The Points Exchange is Checkpoint's core trading platform where users can buy and sell points before they become official tokens. All trades are secured with USDC collateral to ensure both sellers and buyers fulfill their obligations on **settlement**.
...
<img 
  ...
  alt="Checkpoint Exchange Alpha Screen Grab Light"
/>

<img 
  className="hidden dark:block rounded-lg" 
  src="/images/Exchange_Alphascreen_dark.png" 
  alt="Checkpoint Exchange Alpha Screen Grab Dark"
/>

An important part of Checkpoint is that all trades are secured with USDC collateral. This gives the points deposited trading value. The collateral is kept in an **escrow** contract until settlement.
...
### Creating a Sell Offer
    ...
    <Steps titleSize="h4">
      <Step title="Deposit Points">
        Users that want to sell points need to deposit their off-chain points to be on-chain. For this, the user needs to interact with the Oracle and verify that they have points to deposit.
      
        When a user deposits points, they receive a non-transferable NFT that represents the deposit on-chain. This NFT becomes a record for users that they have on-chain points to trade.
      ...
      <Step title="Create a Sell Offer">
        ...
        * **Collateral backing the sell offer**
        ...
        The collateral amount is separate from the price and acts as a security deposit. When creating an offer, users must provide USDC collateral upfront. This collateral is held in escrow until settlement.
      ...
      <Step title="Seller Settlement">
        ...
        * Your existing offers must be settled according to their terms
        ...
        <Info>Offers in the Created status receive allocated collateral minus Fees</Info>
